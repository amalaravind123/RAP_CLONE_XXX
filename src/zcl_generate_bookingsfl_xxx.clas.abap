CLASS zcl_generate_bookingsfl_xxx DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_generate_bookingsfl_xxx IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbookingfl_xxx.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbookingfl_xxx.
*   insert the new table entries
    INSERT ztbookingfl_xxx FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbookingfl_xxx INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
